...
- [ ] Add job/attempt/quota/tracking/partition migrations, scheduler leases/fencing, atomic attempt accounting and configuration-plan hashing. Inventory C. FR-04, NFR-01, NFR-02.
- [ ] Wire every production source/probe request through quota admission and host policy; implement retry/backoff, cancellation, missed-slot expiry and explicit partial-run exit. Inventory B/C. FR-04, FR-10.
- [ ] Implement metric definitions, peak-preserving history, rollups, coverage, average-CCU growth and deterministic replay. Inventory C and B projections. FR-03, NFR-02.
- [ ] Pass `test_history.py` with independent golden integrals, zero baseline, variable cadence, pre-window carry, UTC boundaries, no observations, duplicates and gaps. FR-03, NFR-02.
...
- [ ] Implement dry-run, acknowledgment, enable/disable/run commands; verify changed effective collection plan invalidates acknowledgment and enablement state alone does not. Watch a bounded three-app plan before enabling its schedule. FR-04, FR-10.

## Phase P3: deliver player-statistics alpha

- [ ] Apply `003_enrichment.sql` at the start of P3 for catalog checkpoints/source policies and empty enrichment projections; implement catalog adapter/service, full/incremental reconciliation and bounded cohort policy. Inventory D. FR-05, NFR-01.
...
- [ ] Extend the source policies/projections introduced by P3's `003_enrichment.sql` with the optional Store adapter; validate candidate metadata fields and a paid/free/unavailable one-country sample. Inventory D. FR-07.

...
This subsection identifies the implemented local subset; the remaining sections retain the full-product design targets where they exceed it. Completion evidence belongs to the [walkthrough](build-tasks/initial-release/walkthrough.md). [FR-01–FR-10, NFR-01–NFR-04]
...
The implemented stack is FastAPI/Uvicorn, Pydantic, HTTPX, Psycopg, Jinja2 and standard-library argparse, tested with pytest and Playwright. The initial bounded chart uses local native SVG/JavaScript and an HTML table. ECharts and PostgreSQL time partitions remain possible full-product additions, not installed dependencies. Exact tested versions belong to `uv.lock` and `build/toolchain.lock.json`; first-build evidence addresses GC-B03's runtime portion. The original proposal follows the [FastAPI template interface](https://fastapi.tiangolo.com/advanced/templates/), [PostgreSQL partitioning guidance](https://www.postgresql.org/docs/18/ddl-partitioning.html), and [Apache ECharts](https://echarts.apache.org/en/index.html). [FR-06, NFR-02, NFR-04]
